...
const GAME_LOGGER_ABI = [
  // Events
  'event GameResultLogged(bytes32 indexed logId, address indexed player, uint8 gameType, uint256 betAmount, uint256 payout, bytes32 entropyRequestId, string entropyTxHash, uint256 nftTokenId, uint256 timestamp)',
  
  // Functions
  'function logGameResult(address player, uint8 gameType, uint256 betAmount, bytes memory resultData, uint256 payout, bytes32 entropyRequestId, string memory entropyTxHash, uint256 nftTokenId) external returns (bytes32 logId)',
  'function getGameLog(bytes32 logId) external view returns (tuple(bytes32 logId, address player, uint8 gameType, uint256 betAmount, bytes resultData, uint256 payout, bytes32 entropyRequestId, string entropyTxHash, uint256 timestamp, uint256 blockNumber, uint256 nftTokenId))',
...
/**
 * QIE Game Logger Service
 * Handles logging game results to QIE Blockchain Testnet
 */
export class QIEGameLogger {
  constructor(provider = null, signer = null) {
    this.provider = provider;
    this.signer = signer;
    this.contract = null;
    this.network = QIE_NETWORKS.TESTNET;
    this.contractAddress = QIE_CONTRACTS[this.network].gameLogger;
  ...
  initializeServerWallet() {
    try {
      ...
      console.log('✅ QIE Server wallet initialized:', this.signer.address);
    } catch (error) {
      console.error('❌ Failed to initialize QIE server wallet:', error);
      throw error;
    }
  }
  ...
  initializeContract() {
    try {
      if (!this.contractAddress || this.contractAddress === '0x0000000000000000000000000000000000000000') {
        throw new Error('QIE Game Logger contract address not configured');
      }
      ...
      console.log('✅ QIE Game Logger initialized:', this.contractAddress);
    } catch (error) {
      console.error('❌ Failed to initialize QIE Game Logger contract:', error);
      throw error;
    }
  }
  ...
  /**
   * Log a game result to QIE Blockchain
   * @param {Object} gameData - Game result data
   * @returns {Promise<Object>} Log result with transaction hash and log ID
   */
  async logGameResult(gameData) {
    try {
      if (!this.contract) {
        throw new Error('QIE Game Logger contract not initialized');
      }
      ...
      console.log('📝 Logging game result to QIE:', {
        gameType,
        playerAddress,
      ...
      if (receipt.logs && receipt.logs.length > 0) {
        try {
          ...
          if (parsedLog && parsedLog.name === 'GameResultLogged') {
            logId = parsedLog.args.logId;
          }
        } catch (error) {
          console.warn('Could not parse log ID from event:', error);
        }
      }

      console.log('✅ Game result logged on QIE:', {
        txHash: receipt.hash,
        logId,
      ...
      return {
        success: true,
        txHash: receipt.hash,
        logId: logId || receipt.hash, // Fallback to tx hash if log ID not found
  ...
  async getGameLog(logId) {
    try {
      ...
      return {
        ...
        nftTokenId: Number(log.nftTokenId),
   ...
   */
  async getPlayerHistory(playerAddress, limit = 50) {
    try {
      if (!this.contract) {
        throw new Error('QIE Game Logger contract not initialized');
      }
  ...
  onGameResultLogged(callback) {
    try {
      ...
      const listener = (logId, player, gameType, betAmount, payout, entropyRequestId, entropyTxHash, nftTokenId, timestamp, event) => {
        callback({
          logId,
          player,
...
export const qieGameLogger = new QIEGameLogger();
export default QIEGameLogger;